...
void setup()
{
  ...
  // initialise CAN
  CAN_Init(true);                // place CAN haredware in loopback mode it will receive and acknoledge its own message
  ...
  xTaskCreate(
    ...
    TX_Message[0]='p';
    TX_Message[1]=4;
    //TX_Message[2]=mapindex(7,0);
    TX_Message[2]= 0;
    TX_Message[0]='p';
    TX_Message[1]=5;
    //TX_Message[2]=mapindex(11,3);
    TX_Message[2]=2;
    TX_Message[0]='R';
    TX_Message[1]=6;
    //TX_Message[2]=mapindex(13,2);
    TX_Message[2]=3;
    ...
    for (int iter = 0; iter < 3; iter++) {
      ...
      uint32_t* k = NULL;
      CAN_TX_Task(k);
      //CAN_TX_ISR(); //cause undefined behavior
    }
