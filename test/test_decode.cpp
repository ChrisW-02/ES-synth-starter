...
void decodeText(void *pvParameters)
{
  ...
  while (p==1)
  {
    xQueueReceive(msgInQ, local_RX_Message, 0); // initiate the thread by the availability of data on the queue
...
void setup()
{
  ...
  TaskHandle_t decodeHandle = NULL; // 25.2ms
  xTaskCreate(
